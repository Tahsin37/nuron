...
export default function EditProductPage() {
  ...
  const [form, setForm] = useState<Partial<Product>>({
    name: "", price: "", discount: "", stock_status: "in_stock",
    category: "", delivery_info: "", description: "", notes: "", status: "active"
  });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Update your product knowledge.</p>
        </div>
      </div>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <select value={form.stock_status || "in_stock"} onChange={e => update("stock_status", e.target.value)} className="w-full h-10 rounded-lg border border-zinc-800 bg-zinc-950 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring/20 appearance-none">
      ...
      <Card className="border-border/50">
        <CardContent className="pt-6 space-y-5">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={handleSave} disabled={!form.name?.trim()} className="bg-white text-black hover:bg-zinc-200"><Save className="mr-2 h-4 w-4" /> Save Changes</Button>
          </div>
        </CardContent>
      </Card>
    </div>
